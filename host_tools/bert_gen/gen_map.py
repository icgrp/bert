...
class Mapping:
    def __init__(
            self, word, bit, type, loc, bits, fasmY, fasmINITP, fasmLine, fasmBit, xyz, offset
    ...
    def toStringShort(self):
        return "word={}, bit={}, loc = {}_{}".format(
            self.word, self.bit, self.type, self.loc
        )
...
def createBitMapping(
    ...
    for w in range(words):
        for b in range(bits):
            ...
            mappings.append(
                Mapping(
                    w, b, cell.type, cell.placement, bits, y01, parity, initRow, bbb, xyz, cell.offset
                    # , frameNum,
                    # frameBitOffset
                )
            )
...
if __name__ == "__main__":
    ...
    with open(args.baseDir + '/' + 'list_of_logical.list', 'w') as f_s:
        for logical in all_logical:
            ...
            with open(args.baseDir + '/' + 'mem_' + str(i) + ".info", 'w') as f:
                for m in mappings:
                    f.writelines("{}".format(m.toString()))
                f.write("")
            i = i + 1
        f_s.write("")
