...
Finally, copy the `mydesign.c` and `mydesign.h` files generated by `bert_gen` up above into your application's source directory.

If there are no compiler problems to sort out, the BERT API can now be used.  If there are compiler errors showing, resolve those now.
