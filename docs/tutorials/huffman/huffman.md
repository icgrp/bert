...
1. Now that we have an application project and bsp established, we need to copy the BERT system's source files from `.../bert/embedded/src/bert` into our application project's `src`.  On the left menu, open the application (huffman_demo) and open the src directory.
